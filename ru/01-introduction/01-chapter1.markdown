...
Одной из популярных СУВ данного типа является rcs, которая до сих пор устанавливается на многие компьютеры. Даже в современной операционной системе Mac OS X утилита rcs устанавливается вместе с Developer Tools. Эта улилита основана на работе с наборами патчей (являющимися различием между файлами) между двумя изменениями, хранимыми в специальном формате на диске. Это позволяет пересоздать любой файл на любой момент времени последовательно накладывая патчи.

### Централизованные системы управления версиями ###

Следующей большой проблемой оказалась, что людям надо сотрудничать с разработчиками за другими компьютерами. Чтобы решить её, были созданы централизованные системы управления версиями (ЦСУВ). В таких системах, например CVS, Subversion и Perforce, есть один сервер, в котором хранятся все отслеживаемые файлы, и ряд клиентов, которые получают копии файлов из этого центрального хранилища. Много лет это был стандарт управления версиями (см. рис. 1-2). 
